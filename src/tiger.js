...
class TigerRoller {
    constructor(elem, resize) {
        ...
        //如果大小是可变的就绑定resize事件
        if (resize) window.addEventListener('onorientationchange' in document ? 'orientationchange' : 'resize', () => {
            this.height = this.items[0].clientHeight;
...
class Tiger extends Events {
    constructor(toggle, rollers, options) {
        ...
        for (let i = 0; i < rollers.length; i++) {
            this.rollerQueue.push(new TigerRoller(rollers[i], this.options.resize));
        }
    }
